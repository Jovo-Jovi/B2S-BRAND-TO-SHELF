...
Last task: P02-T08 · Verdict: pending. A tooling task, no application code, no
migration, no schema change, no decision text: `docs/ROADMAP.md` and
`docs/roadmap.html` are now generated from `BUILD_PHASES.md`,
`SESSION_CONTEXT.md`'s own done-steps table and header, `CARRY_FORWARDS.md`
and `DECISIONS.md` §2 and `SCOPE.md` §2, by `scripts/generate_roadmap.py`, and
asserted byte-identical to their own regeneration by `scripts/check_roadmap.py`
— the tenth `docs-integrity` step. The generator embeds no commit sha, branch
or timestamp, on pain of making the byte-comparison permanently unsatisfiable
the moment it is committed (PR-35, landed verbatim after PR-34). Phase status
(DONE/IN PROGRESS/QUEUED) is derived, never hardcoded: DONE requires a PASS
exit-gate row for that phase in the done-steps table, IN PROGRESS is this
file's own `Phase:` header, QUEUED is everything else; a fourth colour, "needs
a decision", marks an open ledger row rather than a phase. Proven on the
required plant-and-revert set — hand-edit `ROADMAP.md`, hand-edit
`roadmap.html`, a `BUILD_PHASES.md` phase name changed without regenerating,
both targets removed, both targets emptied — **7 of 7 caught, every revert
byte-identical** to an in-memory snapshot, never `git checkout --` (PR-26).
`scripts/check_module_spec_tree.py` was run after creating the two new
scripts and two new documents and passed unmodified, because `scripts/` and
`docs/` are named at directory granularity only (OD-H10) and no new directory
was created — `MODULE_SPEC.md` §1 needed no edit. Static conformance set: nine
`docs-integrity` checks and five `guards` → **ten and five, fifteen total**;
two-way (check, premise) set: twenty-six → **twenty-eight** (PR-28,
`check_roadmap.py` joins it with two content-comparison assertions — one per
generated file — 26 + 2 = 28). No carry-forward finding surfaced worth a
ledger row; none opened, per Task 5's own conditional. Ledger unchanged at 120
rows, 36 open. Full detail in the done-steps row below.
