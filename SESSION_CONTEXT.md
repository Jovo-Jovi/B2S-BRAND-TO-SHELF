...
Last task: P02-T10 · Verdict: pending. Tooling, ledger and one precedent —
no application code, no migration, no schema change. **The ownerless ledger
is triaged**, which is the work P02-T09-FIX halted on: **21** open rows carry
the NOT IN THE PLAN status, derived through `generate_roadmap.py`'s own
`collect()` and not from any prior prompt's figure (PR-23), and they split
**10 phase-implied · 2 release-placed · 6 needing an owner decision · 3
genuinely ownerless**, summing to 21. Only the first two groups were amended,
each with the `BUILD_PHASES.md` or `SCOPE.md` §2 sentence that supports it
quoted in the row; **none of the ten is P02**, so none closes before this
phase's gate. Ownerless count after the amendments: 11. The retired
prepare-step numbers `P-07`, `P-10` and `P-12` were superseded, never re-read
as `P07`, `P10` or `P12`. The two-way empty-target probe is invoked by no
workflow and deliberately stays that way; **its cadence is now stated in two
places** — its own module docstring and this file — as every phase exit gate
and every FIX task, with its result cited in that task's report. The proven
figure now carries **the task id that last proved it**, and
`check_stated_counts.py` asserts that id names a done-steps row: eighth scan
target, floor 7 → 8, proven on three plant-and-revert cases, 3 of 3 caught.
Static-assertion count 94 → **99**; the proven count stays 36, no premise
having been added. `.gitattributes` was already correct since P-01 — the real
cause of two files reporting modified against an empty diff was
`core.autocrlf = true` plus a stale index stat cache; **12 working files
renormalised to LF with every committed blob provably unchanged**, one under
`legacy/` excluded by rule. PR-37 landed: a STOP condition names a divergence
that changes the work, never a figure the task derives for itself. Ledger:
125 → 131 rows, 38 open. All fifteen checks green. Full detail in the
...
- **The static conformance set is ten `docs-integrity` checks and five
  ...
  own derivation:
  - The static-assertion count stands at **99** fail() call site(s),
    ...
    duplicated, uncheckable or unmatched task id. No premise was added, so the
    proven count did not move with it, which is the split working as intended.
