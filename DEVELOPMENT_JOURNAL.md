...
2026-08-05 | Opus (heavyweight) | P02 · P02-T04 — session-to-membership resolution under OD-G14 | supabase/migrations/20260805120001_session_tenant_selector.sql, supabase/schema.sql, docs/product/DATA_MODEL.md, __tests__/isolation/tenant-isolation.test.ts, __tests__/isolation/harness.ts, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md | Two malformed selectors shaped like SQL injection were answered 403 by Cloudflare's WAF at the Supabase edge and never reached Postgres, so the assertion that the helper resolves them to null without raising had, for those two values, been testing Cloudflare — a passing proof measuring the wrong component, which is PR-21's shape arriving through infrastructure. Landed as PR-30 and fixed by putting all twelve values through the database directly as well as over HTTP, counting the two paths separately; the same twelve then ran again ten minutes later and the WAF blocked one rather than two, so the block is not even deterministic and a proof that fails on which values are blocked would be flaky by construction. Also: Node's `fetch` rejects a header containing a NUL before sending, so that value never left the harness | the next P02 build task, on the same branch — provisioning or the invitation flow (CF-121); nothing sets `x-b2s-tenant` outside the test harness yet

2026-08-05 | Opus (heavyweight) | P02 · P02-T05 — Member materialisation and Tenant provisioning under OD-G13 | supabase/migrations/20260805120002_member_materialisation_and_tenant_provisioning.sql, supabase/schema.sql, types/database.ts, docs/product/DATA_MODEL.md, __tests__/isolation/tenant-isolation.test.ts, __tests__/isolation/harness.ts, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md | The mechanism question decided the whole task twice. Act one had to be a trigger on `auth.users` rather than an RPC the client calls after signing in, because OD-G13 says authentication creates the Member: an RPC leaves a window in which an authenticated identity holds no `member` row, and — the part that matters — a duplicate address refused inside the trigger aborts the transaction that was inserting the auth row, so the second identity is never created at all. Refusing from an RPC would leave the identity in place with nothing behind it. Act two could not be the ADR-005 `service_role` client for the same class of reason: that client speaks PostgREST, where every request is its own transaction, so tenant + membership + event is three round trips and a failure after the first leaves a tenant with no owner that the deferred `membership_active_owner_required` never catches, firing as it does at the commit of a transaction that already succeeded. One `security definer` function is one transaction by construction, and atomicity was then proven by forcing a failure at two different points rather than by reading the code. Three findings the work produced and a reading would not: `auth.users`'s own `users_email_partial_key` is **partial** on `is_sso_user = false`, so auth's uniqueness covers neither an SSO identity nor a case-differing address and OD-G13's invariant rests on `member.email`'s citext index alone — which is why 24b tests those two routes and treats the ordinary sign-up refusal as evidence about the index in front rather than about the trigger (PR-30); `provision_tenant` passes `base_currency` and `default_locale` through unvalidated while §3.1 fixes both in prose and the table constrains neither, landed as CF-130; and nothing bounds how often an identity may provision, landed as CF-129 with slug squatting named as what stays reachable. One pre-existing expectation moved and is recorded as CF-131 rather than as a paragraph: materialisation is unconditional, so an operator now holds their own `member` row and proof 7's exact set went from none to one. Suite 46 → 56, 56 PASS, and two probe defects each cost a full 9-minute run — `pg_policy.polcmd` is `"char"` and will not concatenate, and `pg_get_function_identity_arguments()` renders parameter names as well as types | the invitation flow (CF-121) is the remaining tenancy write path; nothing calls `provision_tenant()` or sets `x-b2s-tenant` outside the test harness, and no surface bounds provisioning (CF-129)

2026-08-05 | Opus (heavyweight) | P02 · P02-T06 — §11's bypass inventory re-derived and made self-asserting | docs/product/SECURITY_MODEL.md, scripts/check_security_model_bypass.py, .github/workflows/docs-integrity.yml, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md | The defect was not that §11a.1 held a wrong number. It was that the number had no way of being wrong out loud. §11's standing rule (§11.5) re-derives the inventory at every phase exit gate, which means a section can be stale for a whole phase and nobody is required to notice until the gate — and P02-T05 added two `security definer` functions to `public` while its prompt named `DATA_MODEL.md`'s subsections and not this one, the third consecutive task where an unnamed specification went stale. So the deliverable is the check, not the correction. Re-derived rather than read: nine `create ... function public.*` statements in `schema.sql` resolve to eight distinct identities, `current_tenant_id()` being created once and replaced once by migration 13, and the live catalog agrees and puts the whole-catalog total at eleven rather than the nine §11a.1 claimed. Two things a count alone would have missed and writing the justifications found: `materialise_member()` must be `security definer` because `supabase_auth_admin` holds no INSERT on `public.member` and the table carries no INSERT policy, so there is no non-definer path by which authentication could create a Member at all; and §11a.1's claim that each function is "granted explicitly" was never true of `enforce_tenant_active_owner()` — the real split is six granted to `authenticated` and two trigger functions granted to nobody, which assertion 22 had been enumerating ACL by ACL all along without anybody reconciling it against the prose. §11b re-measured with `MEMBER` and unchanged in every respect the tier asks about, including the ten paths and the three-way function reachability that found `vault` at P01-T06-FIX. One value moved: `cli_login_postgres`'s `VALID UNTIL` is not the date §11b.4 recorded but a later one, expired sixty-nine minutes before the reading — the CLI issues a fresh short-dated password on every `db push`, so the role is not permanently dead but dead between pushes, and a LOGIN role one `SET ROLE` from `postgres` was live while migrations 13 and 14 applied. That is CF-133, and it changes one word of §11b.4: revocation is not cosmetic. The check was run against the unedited document before anything was corrected, which is how the counterfactual was answered rather than asserted — it names both missing functions and calls the document the short side, so it would have failed P02-T05's push. Eleven plant-and-revert cases, one per assertion plus both targets removed and emptied, 11 of 11 caught, every revert byte-identical. PR-33 landed on a gap PR-15, PR-18 and PR-24 leave between them: CF-132's supplied text said §11a.1 "enumerated five" and the table held six, a wrong figure about a third artifact that changed nothing about the finding, so it was corrected against the artifact with the original recorded beside it rather than halted over | the invitation flow (CF-121) is still the remaining tenancy write path; §11b's platform half has no static premise and remains the gate's live re-derivation, and CF-133 is the owner's

2026-08-05 | Sonnet (standard) | P02 · P02-T07 — Land OD-G17, OD-G18, OD-H12 and the plan amendment | docs/method/PRECEDENTS.md, docs/product/DECISIONS.md, docs/method/BUILD_PHASES.md, docs/method/DEV_OS.md, docs/product/SECURITY_MODEL.md, docs/product/SCOPE.md, docs/method/CARRY_FORWARDS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md | A land task with two independent verification failures baked into its own prompt, both caught rather than inherited. First, `DEV_OS.md` §2 named four claims as false at HEAD and a fifth — migration discipline, delivered by P01's `check_migration_split.py` the same way the database row was delivered — was false in exactly the same shape and unnamed; all five were corrected row by row against the tree rather than four, with the row-count itself carried into CF-135's closure text so the correction is traceable. Second, the two SESSION_CONTEXT.md figures this task was told to recompute — "six security definer functions" and "twelve checks / sixteen cases" — were already at eight and at fourteen/twenty-six, both landed by P02-T06 itself; re-running the same commands P02-T06 cited found them already correct, so PR-33 was applied in the direction it does not usually run: reporting a supplied figure as already-fixed rather than rewriting a true statement to match a stale prompt. Landing the three decisions was mechanical by comparison — G17 and G18 are policy signed for implementation by a later P02 build task, and H12 adds P09 and moves two items into P03's entry and P05's exit, all verified by count against DECISIONS.md's own register. One self-inflicted failure: the SESSION_CONTEXT.md header narrative named CF-129, CF-130 and CF-135 by id after they closed, which `check_ledger.py` correctly rejects — closed ids may only appear inside the done-steps table — and the header was rewritten to describe the ledger action without the ids. All nine docs-integrity scripts green after the fix, all fourteen of their internal assertions among them | the P02 build task amending `provision_tenant` owes OD-G17's data-layer enforcement and OD-G18's concurrency proof, per CF-129/CF-130's closure text; P03's entry checklist owes CF-109; the invitation flow (CF-121) remains the outstanding tenancy write path

2026-08-05 | Sonnet (standard) | P02 · P02-T08 — docs/ROADMAP.md, generated and self-asserting | scripts/generate_roadmap.py, scripts/check_roadmap.py, docs/ROADMAP.md, docs/roadmap.html, .github/workflows/docs-integrity.yml, docs/method/PRECEDENTS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md | A pure tooling task whose one hard constraint was also its whole design problem: the generator may read the done-steps table, the ledger, the register and the release plan, but may never write a commit sha, a branch name or a timestamp of its own, because any of those three changes the instant the file is committed and would make the check's own byte-comparison against its regeneration permanently false from the moment it first goes green. Status is derived rather than asserted by hand, and stated once so both `docs/ROADMAP.md` and `docs/roadmap.html` agree on the same rule: DONE requires a PASS row for that phase's own exit gate in the done-steps table (only P01 qualifies, at `P01-GATE-RUN3`, since `P01-GATE` and `P01-GATE-RERUN` both read FAIL first), IN PROGRESS is `SESSION_CONTEXT.md`'s own header token, and everything else is QUEUED; a fourth colour, "needs a decision", was resolved as marking an open ledger row rather than a fourth phase state, since the prompt named it for the html legend without naming a fourth derivation rule for phases — read literally rather than invented. One self-caught defect during authoring: the html field renderer used `r"\\s+"` (two literal backslashes, matching nothing) where `r"\s+"` was meant, so a phase's multi-line "Exit standard" field kept its source line break instead of folding to one line in the rendered `<p>`; found by reading the generated output rather than by the check, since a byte-comparison against its own regeneration cannot catch a bug the generator and the check share. Fixed by a `collapse_space()`/`field_html()` helper used everywhere a field crosses the generator/renderer boundary, which also removed three fragile nested-quote regex calls. `check_roadmap.py` proven on the exact plant-and-revert set the prompt named — hand-edit `ROADMAP.md`, hand-edit `roadmap.html`, a `BUILD_PHASES.md` phase name changed without regenerating, both targets removed, both targets emptied — 7 of 7 caught, every revert byte-identical to an in-memory snapshot confirmed by `git status --porcelain` clean afterward, never `git checkout --` (PR-26). `check_module_spec_tree.py` run after creating both scripts and both documents and passed with no edit to `MODULE_SPEC.md` §1, because `scripts/` and `docs/` are named at directory granularity only under OD-H10 and this task created no new directory under either — reported as a negative result rather than narrowed to force a positive one. Two-way (check, premise) set: twenty-six → twenty-eight, `check_roadmap.py` counted by the same one-`fail()`-site-per-assertion rule `check_security_model_bypass.py` set (its own byte-comparison has exactly two distinct premises, one per generated file). No carry-forward finding surfaced that the task itself did not already resolve; none opened. All fifteen checks green, `docs-integrity` now ten steps | the next P02 build task, on the same branch, still owes OD-G17's data-layer enforcement and OD-G18's concurrency proof on `provision_tenant`, and the invitation flow (CF-121); every later task that changes phase state, closes a carry-forward, or adds a done-steps row must regenerate both roadmap outputs in the same commit or `check_roadmap.py` goes red

2026-08-05 | Sonnet (standard) | P02 · P02-T09 — Roadmap phase detail, the shared visual language and role journeys | scripts/generate_roadmap.py, scripts/check_roadmap.py, scripts/check_stated_counts.py, docs/product/ROLE_JOURNEY.md, docs/ROADMAP.md, docs/roadmap.html, docs/method/CARRY_FORWARDS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md | The count that opened the task was also the count that took the longest to close honestly. The prompt's own premise ("check_roadmap.py carries four fail() sites") was true only of the file's state *after* this task's TASK 2 edits landed — before them the file had exactly one — which meant ROW A could not be closed by counting the tree as found; it had to be re-derived at the end, from the file this task itself produces, or the number would already be stale on arrival. A first mechanical pass — a textual regex search for the substring `fail(` — read 7 for `check_roadmap.py`, not 4, because it matched the word inside two docstring sentences and one comment describing the mechanism rather than invoking it; the fix was to parse each `scripts/check_*.py` file's AST and count `Call` nodes whose function is the bare name `fail`, which is immune to prose that merely discusses the function. That count also settled a live inconsistency P02-T08 had introduced without anyone noticing at the time: P02-T08 counted its own single `check_roadmap.py` fail() call site as *two* cases, one per generated file it compares, while P02-T06 had set the precedent at one case per call site — two conventions coexisting under one label. Settled in favour of the mechanical, call-site definition (it is what "enumerate the fail() sites" can actually mean as a reproducible command), giving a re-derived total of 85 across the ten `docs-integrity` scripts; JS `guards` were confirmed out of scope for this figure specifically, since none of the five defines or calls a function named `fail` — a different property from the "fifteen checks total" figure on the same header line, which does count them. `docs/product/ROLE_JOURNEY.md` was authored as a specification rather than prose — one table, 17 rows, seven actors — with every capability traced to a Can/Cannot cell in `TENANCY_MODEL.md` §3 or §5 and every owning phase a real `BUILD_PHASES.md` heading; Operator's and Buyer's rows carry explicit "**Not:**" clauses because those are exactly the two actors most likely to be misread as ordinary tenant roles. `generate_roadmap.py`'s role-journey status derivation reused the note field itself to detect "PENDING A DECISION" (an open CF id or the word "unsigned" beside an OD id), which meant no new column was needed and nothing was hardcoded — confirmed live against CF-32's still-open pre-renumbering citation on Manager's CSV-import row. `check_roadmap.py`'s new conformance checks had to run *before* calling `collect()` rather than after, because `generate_roadmap.py`'s own `role_journey_status()` calls `die()` on an owning phase that does not exist — a hard, process-ending failure with no soft-fail path — so the owning-phase assertion parses the raw table directly and short-circuits before `collect()` can ever reach that line, while the actor and coverage assertions (which do not crash `collect()`) are allowed to run through to the full byte-comparison so a single push can report more than one kind of violation at once. Five new plant-and-revert cases beyond P02-T08's seven, each isolated to fire exactly one assertion and reverted from an in-memory snapshot, never `git checkout --` (PR-26): a role absent from the table (approver, coverage), an unrecognised actor (`Approvers`), a non-existent owning phase (`P99`), `SCOPE.md`'s release blocks reduced below floor (2 of 3, by removing the `**Release 3:**` marker), and `ROLE_JOURNEY.md`'s rows reduced below floor (7 of 17, one row per actor, deliberately keeping every enum value covered so only the floor fired) — 5 of 5 caught, every revert byte-identical, confirmed by `git status --porcelain` clean afterward. The visual language in `docs/roadmap.html` was built to the token set verbatim with no invented colour; `check-no-runtime-cdn`'s own `ROOTS`/`EXTENSIONS` do not reach `docs/*.html` at all, so passing it was proven by construction (no `<link>`, `<script>` or external URL anywhere in the file) rather than by the guard's own report, and that gap between the guard's literal scope and the task's phrasing is reported rather than silently assumed closed. CF-136 and CF-137 opened and closed in this same task, allocated from the live maximum (135) rather than from the prompt. All fifteen checks green | the next P02 build task, on the same branch, still owes OD-G17's data-layer enforcement and OD-G18's concurrency proof on `provision_tenant`, and the invitation flow (CF-121); every later task that changes phase state, closes a carry-forward, adds a done-steps row, or touches `ROLE_JOURNEY.md`'s table must regenerate both roadmap outputs in the same commit or `check_roadmap.py` goes red

2026-08-06 | Sonnet (standard) | P02 · P02-T09-FIX — Split the probe totals, guard the generated page, triage the ownerless ledger | scripts/check_two_way_empty_target.py, scripts/check_stated_counts.py, scripts/check-no-runtime-cdn.mjs, scripts/check_roadmap.py, docs/ROADMAP.md, docs/roadmap.html, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md | The number this task set out to re-derive turned out to have never been derived at all in the sense claimed. "The two-way (check, premise) total" named a dynamic probe — remove a premise, watch it fail, empty it, watch it fail again, revert byte-identical — but every prior task that stated it, including the one that re-derived it most carefully (P02-T09's own AST walk), had measured `fail()` call sites, a property of source code, and called that the proven-behaviour number by the same name. Nobody had lied; the two properties happened to have moved together long enough that conflating them cost nothing, until a task was asked to reprove one and found itself unable to reconstruct the other from any report because no report described it operationally at all. Fixed by actually running it: 37 (check, premise) pairs enumerated across all fifteen checks' premises, 36 proven live on this Windows machine, one gap. The gap taught its own lesson about test infrastructure rather than about the check under test — a PATH-shadowing `git.bat` shim, the standard trick for redirecting a subprocess call without touching the real repository, is silently invisible to a bare `git` invocation on Windows, because Win32 `CreateProcess` appends only `.exe` to an extensionless command name and never consults PATHEXT itself; that resolution belongs to cmd.exe's shell parsing, which `subprocess.run(shell=False)` never invokes. The shim was confirmed bypassed by direct experiment (it printed a fake string; the check received the real repository's ~130-file tracked set instead), not inferred from the check's clean exit — a clean pass from a probe that never reached its target is exactly PR-30's failure shape, restated in test tooling instead of production infrastructure. Recorded as a documented gap, read by code inspection rather than folded into the proven count by assumption. Two smaller defects surfaced and were fixed inside the same landed probe rather than reported and left: `shutil.rmtree` on a scratch git repository hit the M-01 read-only-object-file failure exactly as documented, fixed with the same `onexc` handler that precedent already prescribes; and an earlier throwaway version of the roots-based probe (used for `check-no-hardcoded-literals.mjs` and `check-no-runtime-cdn.mjs`, both of which include the single file `proxy.ts` as a scan root alongside two directories) reported two provably-restored pairs as unrestored, traced to comparing manifests keyed by `os.path.basename()` — which differs between the real path and its `tempfile.mkdtemp()`-named holding copy — rather than by raw bytes; both were false negatives in the probe's own bookkeeping, confirmed against `git hash-object` equal to `git rev-parse HEAD:` on every path the probe touched, and both pairs proved true once the comparison used content instead of a name. `scripts/check-no-runtime-cdn.mjs` widened to `docs/` and `.html` after confirming by grep that `docs/roadmap.html` carries no `<script>`/`<link>` element, so the task's own HALT-on-surfaced-violation condition never had to fire; 7 files became 9. `scripts/check_roadmap.py` gained three assertions that trace `ROLE_JOURNEY.md` through `TENANCY_MODEL.md` §3 directly rather than through the enum alone, closing a real gap the enum-only assertions left open: nothing had ever required that both named non-enum actors actually appear in the table, only that nothing *unrecognised* did, so a `ROLE_JOURNEY.md` that silently dropped every Buyer row would have passed every P02-T09 assertion. `docs/method/REVIEWER_CHAT_INSTRUCTIONS.md` carried an uncommitted, reviewer-authored edit the whole task ran alongside without ever staging, committing or reverting it — confirmed at the end by naming every path explicitly in every commit (the new precedent, PR-36) and checking `git show --stat` afterward names none of it. Task 6 could not run as specified: the NOT-IN-THE-PLAN carry-forward count, enumerated by the generator's own `collect()` rather than by eye, reads 20 at this task's start against a stated 22, and 21 once this task's own new open row landed in the same bucket by construction — the task's own STOP condition for exactly this shape of mismatch fired, and no row was triaged or amended | the next P02 build task, on the same branch, still owes Task 6's ledger triage — re-enumerate the NOT-IN-THE-PLAN set fresh, since it moves every time a row lands there, and classify against whatever it reads at execution — plus OD-G17's data-layer enforcement, OD-G18's concurrency proof on `provision_tenant`, and the invitation flow (CF-121)

2026-08-06 | Sonnet (standard) | P02 · P02-T10 — Triage the ownerless ledger, and three findings | docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, scripts/check_stated_counts.py, scripts/check_two_way_empty_target.py, scripts/check_done_steps_shape.py, scripts/generate_roadmap.py, .gitattributes, .gitignore, docs/ROADMAP.md, docs/roadmap.html, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md | The triage the previous task halted on turned out to be answerable almost entirely from the ledger's own text, and the thing that had made it look unanswerable was a numbering collision nobody had named. Twelve of the twenty-one ownerless rows point at a *document* — `FEATURE_INVENTORY.md`, `PRINT_CONTRACT.md`, `IMPORT_SPEC.md`, `TEMPLATE_MODEL.md`, `RISK_REGISTER.md` — and `BUILD_PHASES.md` assigns each of those to exactly one of the nine phases, so the owner was derivable without inventing anything; what had hidden it is that the same rows also carry `P-07`, `P-10` and `P-12`, which look like phase ids and are not. They are steps of the retired prepare-phase plan, a namespace that still coexists with the build phases inside this repository's own done-steps table (`P-02` and `P01-T02` are both live ids meaning different things), and reading them as `P07`/`P10`/`P12` would have assigned CF-32 to the wrong phase and CF-41 to a phase that does not exist. Every amendment therefore quotes a `BUILD_PHASES.md` sentence naming the document, and explicitly supersedes the stale number rather than reinterpreting it — the difference between resolving an owner and guessing one. Two rows resisted, and were left in the ownerless bucket rather than forced: CF-94's own text splits its owner by root and argues, in so many words, that naming one future owner for all three roots is what produced the defect, so collapsing it to P03 would have contradicted the row to make a bucket tidier; and CF-74's whole owner is `UX_PRINCIPLES.md`, which led to the one finding this task did not expect — that document is one of OD-H7's just-in-time set and `BUILD_PHASES.md` names it in none of the nine phases, so the worked justification for the no-literals rule is owed by nobody. That also decided how CF-73 was handled: it was amended to P08 for its must-not-reproduce half and its `UX_PRINCIPLES.md` half recorded as still unowned in the same clause, because a partial amendment stated as partial is honest while a partial amendment that reads as complete is the exact failure the task warned about. The line-ending finding was misdiagnosed in the prompt in a way that mattered: `.gitattributes` was not missing, it had been correct since P-01, and `git check-attr` confirmed the LF rule was in force on both affected paths the whole time — the cause was `core.autocrlf = true` in the local git configuration plus an index whose stat cache could not be refreshed, which is why the files reported modified while `git diff` was empty and `git diff-files` showed a null destination hash. The instructive part was that the first refresh attempt appeared to fail (`update-index --refresh` printing "needs update" for every path) and in fact succeeded the moment it ran unsandboxed, so the "failure" was the tool environment and not git; and that thirteen files carried CRLF rather than the two the prompt named, of which one is under `legacy/` and was left alone because the freeze outranks tidiness. Proving nothing was lost mattered more than fixing it: `git hash-object --no-filters` after each rewrite equals `git rev-parse HEAD:` for all twelve, which is the direct statement that only working-tree endings moved. The provenance change is small and closes a class: the proven two-way empty-target figure was already asserted against the probe's own `PROVEN_PAIRS` list, which prevents drift but says nothing about *when the probe last ran* — and since the probe is invoked by no workflow, and deliberately stays that way because a push that plants and reverts real files is worse than the gap it fills, the figure had no way to age visibly. It now names the task that last proved it and the check refuses an id no done-steps row carries. One plant miscarried and is recorded rather than smoothed over: the duplicate-provenance case was first planted without the phrase prefix the regex requires, so the check ignored it correctly and the case had to be re-planted before it proved anything — a probe that fails to plant is a probe that proves nothing, and it looks identical to a check that missed. The last finding was found by the roadmap refusing to build, and it is the one worth keeping: `check_done_steps_shape.py` asserted *fewer* than four columns per done-steps row and was structurally blind to a row split into more, so P02-T09-FIX's own row — which described a table defect by quoting a fragment carrying two unescaped pipes — had been sitting in the state file as six columns for a whole task, rendering a truncated description and somebody else's commit sha into `docs/ROADMAP.md`, past ten scripts and five guards. Two things made it invisible rather than one: the column-count test only ever looked in the short direction, and the commit-column exemption that PR-17 correctly grants the last row meant the newest row is the one nothing examines until something lands after it, so a defect in a *different* column of that row inherited an exemption written for the commit. Both parsers now demand exactly four. The proof arrived by accident and is better than the one that was planned: this task's own done-steps row, in the sentence describing the defect, reproduced it — the strengthened check failed on the first run after being strengthened, which is the shortest possible demonstration that the assertion is live, and it is reported rather than quietly reworded because a check that catches its author on the day it lands is evidence and a check that never fires is not. Planted afterward in both directions anyway, 2 of 2 caught by check and generator alike, reverts byte-identical; the truncated-row plant needed re-planting first, because the anchor string was passed through PowerShell, which eats the backticks the row contains and left the plant matching nothing — already recorded at M-01 ("an ad-hoc `python -c` one-liner is safe only while it contains no `<`, `>`, `(`, `)` or backtick"), and its consequence is the part worth naming: a plant that silently fails to apply reports as a check that MISSED, so the two are indistinguishable from the probe's output alone and a miss must always be re-planted before it is believed | the next P02 build task, on the same branch, still owes OD-G17's data-layer enforcement, OD-G18's concurrency proof on `provision_tenant`, and the invitation flow (CF-121); CF-146 is the reviewer's, and the twelve rows still ownerless are honestly so rather than untriaged

2026-08-06 | Sonnet (standard) | P02 · P02-T11 — Fold the roadmap: collapsible phases and progress log | scripts/generate_roadmap.py, scripts/check_roadmap.py, scripts/check-data-boundary.mjs, docs/ROADMAP.md, docs/roadmap.html, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md | The fold itself was mechanical: every phase card and every progress-log entry in `docs/roadmap.html` became a `<details>` whose `<summary>` is its old header, zero JavaScript, the marker suppressed both ways and a CSS-border chevron rotating on `[open]`; the one genuinely new derivation was the log summary's date, since `SESSION_CONTEXT.md`'s done-steps table carries no date column at all. `DEVELOPMENT_JOURNAL.md` — this file — became the generator's seventh input for that one purpose only, matched by the earliest dated line naming a given Step as its own token, with a hyphen-aware boundary so `P02-T09` can never match inside `P02-T09-FIX`; the generator's docstring states plainly why `docs/ROADMAP.md` does not fold and `docs/roadmap.html` does, so the two outputs' one deliberate difference is documented rather than left to be rediscovered as a bug. The task that mattered more was proving the fold cannot silently flatten, and it caught its own author in the act: the four new `check_roadmap.py` assertions passed every on-disk plant-and-revert case, but only because the pre-existing byte-compare-against-regeneration was catching each plant first, for the pre-existing reason. One case — a whole phase `<details>` deleted — should have dropped the true element count from 53 to 52, one below the floor, but the check's own count read 54 before the plant and 53 after, an off-by-one that hid the regression behind a floor that happened to still be met. The cause was the new tag-scanning regex reading the file as one flat token stream: a CSS comment inside the page's own `<style>` block, written to *document* the fold ("all native `<details>`/`<summary>` plus CSS"), was itself valid-looking `<details>`/`<summary>` markup to a regex that does not know what a comment is, and it inflated the true count by exactly one, permanently, whether or not any plant was in play. Nothing on disk could have surfaced this on its own, because every on-disk plant is also a byte-compare mismatch and the run fails either way; it took a second pass, testing `check_fold_conformance()` in isolation against hand-built HTML/Markdown fragments with the byte-compare never in the call path, to prove the new assertions were sound rather than merely present — 8 cases, 7 targeted and one clean baseline, all correct once `STYLE_BLOCK_RE` strips `<style>...</style>` before tokenizing. Landed as PR-38: a structural assertion added beside an existing byte-compare must be proven with the byte-compare removed, not merely proven to fire, because every on-disk plant that also breaks the byte-compare will pass regardless of whether the new assertion works at all. The smaller finding, `check-data-boundary.mjs` stating no floor at all against PR-27's own standing rule, was the same CF-94 shape one guard over — fixed the same way, two floors at the true counts of 10 files and 3 import sites, each proven by a one-above-truth plant and reverted clean. CF-148 opened and closed in the same task; PR-38 the only precedent this task added. Static-assertion count 99 → 104, entirely inside `check_roadmap.py`'s new function; the proven two-way empty-target count does not move, no premise added, and per its own stated cadence the probe itself was not re-run — this is neither a phase exit gate nor a FIX task. Ledger 132 → 133 rows, 38 open unchanged, since the one new row opened and closed inside this same task. All fifteen checks green | the next P02 build task, on the same branch, still owes OD-G17's data-layer enforcement, OD-G18's concurrency proof on `provision_tenant`, and the invitation flow (CF-121); every later task that changes phase state, closes a carry-forward, or adds a done-steps row must regenerate both roadmap outputs in the same commit or `check_roadmap.py` goes red

2026-08-31 | Grok 4.6 (heavyweight; prompt named Opus) | P02 · P02-T12 — G17's constraints, G18's bounds, and the invitation flow | supabase/migrations/20260831120001_tenant_locale_and_currency_constraints.sql, supabase/migrations/20260831120002_provision_tenant_bounds.sql, supabase/migrations/20260831120003_invitation_by_email.sql, supabase/schema.sql, docs/product/DATA_MODEL.md, docs/product/DOMAIN_MODEL.md, docs/product/SECURITY_MODEL.md, types/database.ts, __tests__/isolation/harness.ts, __tests__/isolation/tenant-isolation.test.ts, docs/method/CARRY_FORWARDS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | Three migrations, 14 become 17. G17 as CHECKs not enums so P07 can drop them for Settings FKs; both directions proven (27a, 27b). G18 via pg_advisory_xact_lock because a partial unique index cannot express at-most-three or a rolling window; 28d ran two concurrent provision_tenant RPCs (Promise.all, 88ms) and exactly one succeeded. G16 as public.invitation plus accept_invitation(uuid) plus caller_email_is_verified(); two new security definer functions, §11a.1 eight to ten, catalog eleven to thirteen. CF-121 closed on 29a–29g. CF-129 proved by 28a–28e, CF-130 by 27a/27b, not reopened. Isolation ledger 70 expected — 70 PASS, 0 FAIL, 0 LOST, all 56 retained. Four ADR-012 counts zero. Empty-target probe 36 of 36, KNOWN_GAPS 1, provenance left at P02-T09-FIX. No new CF or PR id | READINESS (OD-H8) then the P02 exit gate on phase/02-tenancy-and-access; no UI, routes or forms; no PR

2026-08-31 | Grok 4.6 (heavyweight; prompt named Opus) | P02 · P02-T13 — ConsentGrant and Operator reach: the decision, and the absence proven | supabase/migrations/20260831120004_operator_no_api_write.sql, supabase/schema.sql, docs/product/DECISIONS.md, docs/product/SECURITY_MODEL.md, docs/product/TENANCY_MODEL.md, docs/product/DATA_MODEL.md, docs/product/ROLE_JOURNEY.md, docs/method/CARRY_FORWARDS.md, docs/method/B2S_PREPARE_PHASE.md, docs/archive/2026-08/VOCABULARY_DRAFT.md, scripts/check-enum-keys.mjs, __tests__/isolation/harness.ts, __tests__/isolation/tenant-isolation.test.ts, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | OD-G19 landed. Catalog at cce71e7 had service_role holding INSERT/UPDATE/DELETE/TRUNCATE on public.operator, so the signed "no API role holds INSERT, UPDATE or DELETE" clause was false; resolved by migration 18 (revoke all, grant select) rather than carving service_role out. TRUNCATE revoked with the three named verbs. Isolation 80 expected — 80 PASS, 0 FAIL, 0 LOST, all 70 retained, line D all zeroes. CF-149 closed (enum-keys floors 12 and 4, plant-and-revert, PROVEN_PAIRS stays 36). CF-150 opened (consent_scope vacuous until a second value). ROLE_JOURNEY extended the existing OD-G10 Not: clause, no new row. Types CLI-regenerated, byte-identical. Six ADR-012 counts zero | Auth wiring next on phase/02-tenancy-and-access; READINESS (OD-H8) then the P02 exit gate; no Operator provisioning endpoint; no PR
2026-09-01 | Grok 4.6 (heavyweight; prompt named Opus) | P02 · P02-T14 — Supabase Auth wiring: the sign-in surface, features/, and the zod guard's target | app/[locale]/(public)/sign-in/page.tsx, app/[locale]/(public)/callback/route.ts, features/access/, lib/supabase/session.ts, proxy.ts, scripts/check-zod-coverage.mjs, scripts/check_two_way_empty_target.py, scripts/check-no-hardcoded-literals.mjs, scripts/check-no-runtime-cdn.mjs, scripts/check-service-import.mjs, scripts/check-data-boundary.mjs, .github/workflows/ci.yml, docs/product/MODULE_SPEC.md, docs/product/SECURITY_MODEL.md, docs/method/CARRY_FORWARDS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html, vitest.config.mts, app/[locale]/dictionaries/en.json, app/[locale]/dictionaries/ar.json | features/access added citing SCOPE Auth and Access. Sign-in email-and-password and Google. Refused-materialisation contract in SECURITY_MODEL section 2 item 6. check-zod-coverage sixth guard, 4 of 4 plant-and-revert. PROVEN_PAIRS 36 to 37, provenance P02-T14, 37 of 37, KNOWN_GAPS 1. Isolation 80 PASS, 0 FAIL, 0 LOST, PART 0 counts zero. CF-151 opened not resolved. CF-94 features half amended, components stays open. 38 plus 1 minus 0 = 39 open, ledger 135 to 136. Static set sixteen. REVIEWER_CHAT unstaged | READINESS (OD-H8) then the P02 exit gate on phase/02-tenancy-and-access; CF-151 is the readiness task's; no PR
2026-09-01 | Grok 4.6 (standard; prompt named Sonnet) | P02 · P02-READINESS — mechanically checkable state of Phase 02, reported as a LIST | scripts/check_done_steps_shape.py, docs/method/CARRY_FORWARDS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | PART 0 both counts zero. Sixteen checks exit 0. lint/typecheck/test/build exit 0. CI set equals PART 1. CF-151 closed on PR-17 first branch: last-row verdict placeholder, follow-up fills sha only, two new fail() sites, 104 to 106, 3 of 3 plants caught on a temp copy. Four P02-owned rows remain open (CF-01, CF-11, CF-72, CF-75). Isolation suite not run. 39 plus 0 minus 1 = 38 open, ledger 136. REVIEWER_CHAT unstaged | close or retarget those four rows before the P02 exit gate; no PR

2026-09-01 | Grok 4.6 (standard; prompt named Sonnet) | P02 · P02-T15 — clear the four P02-owned rows readiness found, and the two floors it exposed | docs/requirements/extracts/REPORT.md, AGENTS.md, .cursor/rules/b2s-devos.mdc, scripts/check_stated_counts.py, scripts/check_migration_split.py, scripts/check_done_steps_shape.py, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | PART 0 both counts zero. REPORT.md annotated not corrected: banner (15 and 9 hits excluding the banner) and §3.3 citing EXTRACT_DESIGN_TOOLS.md §8.7; §8.5 confirmed against EXTRACT_INVOICE_PRO.md. Both rules files restored with Guard column; print NOT YET ENFORCED owner P06; HTML-injection is the lint rule. Ninth scan target on check_stated_counts, floor 7 paths. Migration-split floors 18 and 1514; pending frozen to 25 Step ids (prompt said 23). 8 of 8 plants CAUGHT, every revert SHA-256-identical. CF-01, CF-11, CF-72, CF-75 closed. CF-152 and CF-153 opened and closed. 38 plus 0 minus 4 = 34 open, ledger 136 to 138. Static-assertion 106 to 117. PROVEN_PAIRS stays 37, provenance P02-T14. Isolation suite not run. REVIEWER_CHAT unstaged | P02 exit gate on phase/02-tenancy-and-access; inbox CF-92, CF-126, CF-133; no PR
2026-09-01 | Grok 4.6 (heavyweight; prompt named Opus) | P02 · P02-GATE — Phase 02 exit verification | SESSION_CONTEXT.md, docs/method/CARRY_FORWARDS.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | 0 hard failures. Isolation 80 PASS, 0 FAIL, 0 LOST, line D all zeroes. Section 11 both tiers match live catalog; 11b.5 search_path pin is a document correction (CF-155). Types 17742 byte-identical. Two-way 37 of 37, KNOWN_GAPS 1. 18 of 18 plants CAUGHT. Four concurrent provision from zero owned not in the suite (CF-154). CF-92, CF-126, CF-133 re-derived and left OPEN with this gate retired as owner. 34 plus 4 minus 0 = 38 open, ledger 138 to 142. REVIEWER_CHAT unstaged | owner opens the consolidated PR per BRANCHING section 3; containment git log main..branch empty before delete; no PR from this task
2026-09-02 | Grok 4.6 (standard; prompt named Sonnet) | P03 · P03-ENTRY — Phase 03 entry checklist, two unsigned forks | SESSION_CONTEXT.md, docs/method/CARRY_FORWARDS.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | PART 0 containment: branch phase/02-tenancy-and-access, merge da0fe8b, tree 0584ffbce5ffbbd84ab464d877a3d8d9bd2755e2; git log main..phase/02-tenancy-and-access --oneline printed nothing; trees equal. Branch not deleted. 38 open via generate_roadmap.collect() at da0fe8b; CF-158 opened. 38 plus 1 minus 0 = 39 open, ledger 142 to 143. P02-GATE verdict PASS. Two briefs assembled not signed (CF-158 object storage, CF-109 staging). Catalog task has not run. Isolation suite not run. REVIEWER_CHAT unstaged | owner signs the two forks; do not cut phase/03-brand-and-onboarding until signed; owner deletes phase/02-tenancy-and-access on this containment record; no PR
2026-09-02 | Grok 4.6 (standard; prompt named Sonnet) | P03 · P03-FIX-01 — types-drift red on main, CI-conclusion precedent | types/database.ts, docs/method/PRECEDENTS.md, docs/method/CARRY_FORWARDS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | Fail on drift, not secrets: job 100211559716 run 33618944715. Local regen 17752 vs 17742, catalog prefix identical at 13199, helper parentheses only. CLI-regenerated, not hand-edited. CF-159 opened and closed by PR-39. types-drift first red this episode 2026-09-02 on 154016d; did not predate the P02 merge (da0fe8b types-drift success 09:43 UTC). Deviation: PART 1 byte mismatch looked like the halt, but the live schema had not moved (PR-37). Isolation suite not run. REVIEWER_CHAT unstaged | owner signs the two forks; do not cut phase/03-brand-and-onboarding; report the CI conclusion of this push per PR-39; no PR
2026-09-02 | Grok 4.6 (heavyweight; prompt named Opus) | P03 · P03-T01 — HALT at PART 0: no staging project, no staging secrets | SESSION_CONTEXT.md, docs/method/CARRY_FORWARDS.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | STOP 1. Org jiovanny plan pro. One ACTIVE_HEALTHY B2S project (b2s-production); no staging project; no staging branch; no STAGING-named repository secret. Independent production counts both [{"count":0}] at 2026-09-02T13:22:41Z / 13:22:42Z. Advisory done-steps 52 vs enumerated 53. No OD/ADR landed, no phase branch, isolation suite not run. CF-109 amended, stays open. 39 plus 0 minus 0 = 39 open, ledger 144. REVIEWER_CHAT unstaged | owner creates b2s-staging on the Pro org and sets SUPABASE_STAGING_ACCESS_TOKEN plus SUPABASE_STAGING_PROJECT_ID; then resume P03-T01; no PR
2026-09-17 | Grok 4.6 (heavyweight) | P03 · P03-STAGING-CREATE — owner act: create b2s-staging, keep production named | SESSION_CONTEXT.md, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, DEVELOPMENT_JOURNAL.md | b2s-staging ACTIVE_HEALTHY on the Pro org in eu-central-2. b2s-production name and role unchanged (PR-40). Four STAGING-named repository secrets set; production six last-updated dates unchanged. Existing access token reaches both project refs (HTTP 200). Staging virgin: 0 public tables, 0 migrations. Production tenant and auth.users both [{"n":0}]. No OD/ADR landed, no migrations applied, isolation suite not run. CF-109 amended, stays open. 39 plus 0 minus 0 = 39 open, ledger 144. Workspace already on phase/03-brand-and-onboarding (P03-T01 halt plus follow-up, two commits ahead of origin/main); this session did not cut it. REVIEWER_CHAT unstaged | resume P03-T01 on that branch; apply the 18-migration chain to virgin staging and compare catalogs; no PR
2026-09-17 | Grok 4.6 (standard; prompt named Sonnet) | P03 · P03-T01a — land the staging environment record and pending method edits | SESSION_CONTEXT.md, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | CF-160 opened and closed as the missing record of b2s-staging. CF-161 opened on the Postgres patch inversion. Prior session's four method files committed including PR-40. P03-T01 verdict PASS. 39 plus 1 minus 0 = 40 open, ledger 144 to 146. Isolation suite not run. REVIEWER_CHAT unstaged | P03-T01-RESUME on phase/03-brand-and-onboarding; apply the 18-migration chain to virgin staging; no PR
2026-09-17 | Grok 4.6 (heavyweight; prompt named Opus) | P03 · P03-T01-RESUME — land OD-G20, OD-H13, ADR-013; stand staging; move isolation venue | docs/product/DECISIONS.md, docs/product/ADR.md, docs/product/ARCHITECTURE.md, docs/product/SECURITY_MODEL.md, docs/method/BUILD_PHASES.md, docs/method/CARRY_FORWARDS.md, .github/workflows/ci.yml, .github/workflows/isolation.yml, __tests__/isolation/harness.ts, __tests__/isolation/tenant-isolation.test.ts, vitest.isolation.config.mts, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | Register 92 to 94. ADR-012 superseded, body intact. 18 migrations applied to virgin staging, catalogs MATCH on B2S-owned public objects; types 17752 byte-identical. Isolation staging-only with no production fallback; negative proof named four absent staging variables. Suite 82 PASS, 0 FAIL, 0 LOST, line D zero; production and staging counts n=0 after. Assertion 31 live name b2s-staging. CF-92, CF-109, CF-154, CF-155, CF-156, CF-158 closed; CF-162, CF-163 opened; CF-161 stays open. 40+2-6=36 open, ledger 146 to 148. Static set stays 16. REVIEWER_CHAT unstaged | next P03 build task on phase/03-brand-and-onboarding; error visibility (CF-162) before wizard content; no PR
2026-09-19 | Grok 4.6 (heavyweight; prompt named Opus) | P03 · P03-T02 — restore per-push isolation coverage, MCP containment guard | .github/workflows/isolation.yml, .github/workflows/docs-integrity.yml, .cursor/mcp.json, scripts/check_mcp_containment.py, scripts/check_two_way_empty_target.py, scripts/check_stated_counts.py, AGENTS.md, .cursor/rules/b2s-devos.mdc, docs/product/ARCHITECTURE.md, docs/product/ADR.md, docs/method/CARRY_FORWARDS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | Push trigger broadened to every branch; concurrency group retained. MCP file tracked with structural guard; 5 of 5 plants CAUGHT; two-way 39 of 39, KNOWN_GAPS 1. Static set 16 to 17, fail() 117 to 137. CF-109 stays closed; CF-164 closed; CF-165 opened. 36 plus 1 minus 0 = 37 open, ledger 148 to 150. REVIEWER_CHAT unstaged | next P03 build task on phase/03-brand-and-onboarding; staging MCP posture is the owner's; no PR
2026-09-19 | Grok 4.6 (standard; prompt named Sonnet) | P03 · P03-T03 — make updated_at real: maintenance trigger, assertion, CF-93 last gap | supabase/migrations/20260919120001_updated_at_maintenance.sql, supabase/schema.sql, docs/product/DATA_MODEL.md, scripts/check_data_model_schema.py, scripts/check_migration_split.py, __tests__/isolation/harness.ts, __tests__/isolation/tenant-isolation.test.ts, docs/method/CARRY_FORWARDS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | Five tables derived live. Function not security definer. Types 17752 byte-identical. Isolation 84 PASS, 0 FAIL, 0 LOST, line D zero; production n=0 after. fail() 137 to 138. PROVEN_PAIRS stays 39. CF-93 closed. 37 plus 0 minus 1 = 36 open, ledger 150. Prompt's UPDATE-grant and six-tables figures were false of the catalog. REVIEWER_CHAT unstaged | next P03 table-creating task on phase/03-brand-and-onboarding; no PR
2026-09-22 | Grok 4.6 (heavyweight; prompt named Opus) | P03 · P03-T04 — land Brand, Asset and TranslationKey schema | supabase/migrations/20260922120001_brand_asset_enums.sql, supabase/migrations/20260922120002_translation_tables.sql, supabase/migrations/20260922120003_asset_tables.sql, supabase/migrations/20260922120004_brand_tables.sql, supabase/migrations/20260922120005_brand_asset_policies_and_grants.sql, supabase/schema.sql, types/database.ts, __tests__/isolation/harness.ts, __tests__/isolation/tenant-isolation.test.ts, scripts/check_data_model_schema.py, scripts/check_migration_split.py, scripts/check-enum-keys.mjs, SESSION_CONTEXT.md, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | Spec first at abd3efd. Twelve new tables not eleven. Isolation 94 PASS, 0 FAIL, 0 LOST, D last at zero. Types 17752 to 42063, CLI, no BOM. Production apply via db push; dump needs Docker and was empty, catalog JSON used. Catalogs agree, 0 chain-derived diffs. Floors moved as stated. PROVEN_PAIRS stays 39. 36 plus 0 minus 0 = 36 open. REVIEWER_CHAT unstaged | next P03 build task on phase/03-brand-and-onboarding; no wizard; no PR
2026-09-22 | Grok 4.7 (standard; prompt named Sonnet) | P03 · P03-T05 — recovery point without Docker, two decisions, two method corrections | docs/product/DECISIONS.md, docs/product/ADR.md, docs/method/PRECEDENTS.md, docs/method/BUILD_PHASES.md, docs/method/CARRY_FORWARDS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | PART 0 Management API both counts [{"n":0}] at 15:36:32Z and 15:36:35Z. db query --linked against production returned LegacyDbConfigIpv6Error and was not the count; link file restored. Register 94 to 96, OD-H14 and OD-G21. ADR-013 amended beside the text. PR-41 and PR-42. pg_dump 17.11 user-local, Authenticode NotSigned. CF-146 closed, CF-74 and CF-73 amended, CF-166 opened and closed, CF-167 opened. 36 plus 1 minus 1 = 36 open, ledger 150 to 152. Isolation suite not run. REVIEWER_CHAT unstaged | reviewer authors UX_PRINCIPLES.md against OD-G21, then the design session for the catalog; CF-167 is the reviewer's branching gap; no PR

2026-09-23 | Grok 4.7 (standard; prompt named Sonnet) | P03 · P03-T06 halt — draft UX section 3 contradicted CALC_SPEC R1-25 | none | Halted before the first edit. PASS. The draft required Western digits for money in both locales; R1-25's Expected line rendered Arabic money with Arabic-Indic digits. D8 was not the conflict. Nothing written. | re-run once R1-25 is amended beside its text and section 3 defers
2026-09-23 | Grok 4.7 (standard; prompt named Sonnet) | P03 · P03-T06 — Latin-digit locale definitions, UX_PRINCIPLES.md, catalog gates | docs/product/UX_PRINCIPLES.md, docs/product/CALC_SPEC.md, docs/product/ADR.md, docs/method/CLAUDE_PROJECT_INSTRUCTIONS.md, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | PART 0 one hit, CALC_SPEC.md:646. R1-25 amended beside the original Expected line, diff +28/−0. PR-43. CF-74 stays open. CF-73 P03 half discharged. Gates CF-168 to CF-180 open. Binaries CF-181 open. CF-182 and CF-183 closed. 36 plus 16 minus 2 = 50 open, ledger 152 to 168. en/ar 19 keys, same set. REVIEWER_CHAT unstaged. Isolation suite not run | reviewer authors DESIGN_SURFACE.md; catalog landing task owns CF-94 and CF-168 through CF-180; CF-167 is the reviewer's branching gap; CF-181 is the owner's; no PR
2026-09-23 | Grok 4.7 (heavyweight; prompt named Opus) | P03 · P03-T07 — land DESIGN_SURFACE, OD-G22, OD-G23, ADR-014, jsdom and axe-core | docs/product/DESIGN_SURFACE.md, docs/product/CALC_SPEC.md, docs/product/BRAND_CONFIG.md, docs/product/UX_PRINCIPLES.md, docs/product/DECISIONS.md, docs/product/ADR.md, docs/method/CARRY_FORWARDS.md, package.json, package-lock.json, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | Staged file 63770 bytes, SHA-256 081eec70fe2825476a9d3e8b915b72a0a2ab66e4353ac0583f13e1550e1746fb. PART 1 no different value. Register 96 to 98. Pins jsdom 30.1.1 MIT and axe-core 4.13.0 MPL-2.0. Probe vitest 4.1.10, per-file jsdom, global environment node. Console: Not implemented: HTMLCanvasElement's getContext() method: without installing the canvas npm package (twice). en and ar identical rule ids. violations []. incomplete (3), each node failureSummary "Fix all of the following: Axe encountered an error; test the page for this type of problem manually": color-contrast impact serious node h1 Sign in / تسجيل الدخول; landmark-one-main impact moderate target html; page-has-heading-one impact moderate target html. The view contains main and h1; those two rules errored rather than observed an absence. passes (17): aria-hidden-body, autocomplete-valid, button-name, document-title, empty-heading, form-field-multiple-labels, heading-order, html-has-lang, html-lang-valid, label-title-only, label, landmark-main-is-top-level, landmark-no-duplicate-main, landmark-unique, nested-interactive, region. inapplicable (70): accesskeys, area-alt, aria-allowed-attr, aria-allowed-role, aria-braille-equivalent, aria-command-name, aria-conditional-attr, aria-deprecated-role, aria-dialog-name, aria-hidden-focus, aria-input-field-name, aria-meter-name, aria-progressbar-name, aria-prohibited-attr, aria-required-attr, aria-required-children, aria-required-parent, aria-roles, aria-tab-name, aria-text, aria-toggle-field-name, aria-tooltip-name, aria-treeitem-name, aria-valid-attr-value, aria-valid-attr, avoid-inline-spacing, blink, bypass, definition-list, dlitem, duplicate-id-aria, empty-table-header, frame-focusable-content, frame-tested, frame-title-unique, frame-title, html-xml-lang-mismatch, image-alt, image-redundant-alt, input-button-name, input-image-alt, landmark-banner-is-top-level, landmark-contentinfo-is-top-level, landmark-no-duplicate-banner, landmark-no-duplicate-contentinfo, link-in-text-block, link-name, list, listitem, marquee, meta-refresh, meta-viewport-large, meta-viewport, object-alt, presentation-role-conflict, role-img-alt, scope-attr-valid, scrollable-region-focusable, select-name, server-side-image-map, skip-link, summary-name, svg-img-alt, tabindex, table-duplicate-name, td-headers-attr, th-has-data-cells, valid-lang, video-caption, no-autoplay-audio. Client bundle .next/static 10 files, 0 hits for jsdom or axe-core; .next/server 175 files, 0 hits. Mechanical: 21 blocks, 0 state problems, 55 n/a all reasoned, 0 raw px or hex in blocks, 0 glossary-noun hits, chrome neutrals 27 ok. Findings not corrected: weight-500 undefined; text-xs text-sm text-xl are scale steps not --b2s-text-* tokens; enumerated AA pairs 56 all passing against a stated seventy; dark border-control on sunken computed 4.06 against a published 3.93, and raised dark is 3.33, still at least 3.0. CF-184 opened and closed. CF-185 opened. CF-140 and CF-176 amended. 50 plus 1 minus 0 = 51 open, ledger 168 to 170. Isolation not run. REVIEWER_CHAT unstaged. Probe test not committed. | catalog landing task builds from DESIGN_SURFACE.md; CF-185 before P06; CF-167 is the reviewer's branching gap; no PR

2026-09-25 | Grok 4.7 (standard; prompt named Sonnet) | P03 · P03-T08 — corrections, fonts, tokens, four static gates | docs/product/DESIGN_SURFACE.md, docs/product/UX_PRINCIPLES.md, docs/method/CLAUDE_PROJECT_INSTRUCTIONS.md, docs/method/BUILD_PHASES.md, docs/method/BRANCHING.md, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, app/globals.css, app/[locale]/layout.tsx, public/fonts/IBMPlexSans-Regular.woff2, public/fonts/IBMPlexSans-Medium.woff2, public/fonts/IBMPlexSans-SemiBold.woff2, public/fonts/IBMPlexSansArabic-Regular.woff2, public/fonts/IBMPlexSansArabic-Medium.woff2, public/fonts/IBMPlexSansArabic-SemiBold.woff2, public/fonts/OFL.txt, public/fonts/PROVENANCE.md, scripts/check-logical-properties.mjs, scripts/check-catalog-parity.mjs, scripts/check-catalog-duplicates.mjs, scripts/check-token-values.mjs, scripts/check-no-runtime-cdn.mjs, scripts/check_two_way_empty_target.py, .github/workflows/ci.yml, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | Node v24.11.1 against .nvmrc 24. Blocks 21, n/a 55 all reasoned, contrast pairs 56 all at or above their floor, chrome neutrals achromatic, referenced tokens defined once brand short-names are read from --brand-*. Fonts: @ibm/plex-sans@1.1.0 https://github.com/IBM/plex/releases/download/@ibm/plex-sans@1.1.0/ibm-plex-sans.zip IBMPlexSans-Regular.woff2 63020 ba711a3085ff9f27440b6b9c4550cfc47c97bf36591d5da958b975bb3add8c1a; Medium 66740 5660f8a658f8bb50dbc005232f885eadffd2bc1c235c4f6fbb63469d1f9cde6d; SemiBold 67060 f78048030eab62e860efa39a0df79e2e5581bf122eb95b9bc42c0b8a4988d205. @ibm/plex-sans-arabic@1.1.0 https://github.com/IBM/plex/releases/download/@ibm/plex-sans-arabic@1.1.0/ibm-plex-sans-arabic.zip Regular 71904 74112e1b7bf2e2c547989a4e947a60cba6cd1f9156e0065670134636e717199d; Medium 75936 c72f6986f50ded745d06e71e508096209c52a55a6b3a81029954e33c52ba1dd2; SemiBold 76408 869b2acd0a9660607a2025f0fe1c640110bf23d33417509328f07722922b8063. OFL.txt 4456 7e6b2818edbd8f6a01ae80641cc8f16a51080d08fb4e532be3a0b6f74adb07da. CDN check sees the six local woff2 urls. signIn title/submit justified not merged. 51 + 3 - 8 = 46. Static set 21. PROVEN_PAIRS 45. | P03-T09 catalog primitives; CF-187 with CF-176; no PR
2026-09-25 | Grok 4.7 (standard; prompt named Sonnet) | P03 · P03-T08-FIX — rewrite the two static checks the plants escaped | scripts/check-logical-properties.mjs, scripts/check-token-values.mjs, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | PART 0 no open PR against main. Node v24.21.0 on PATH via the nvm-root shim; standalone v22.12.0 still installed; v24.11.1 not found. Logical set 60 from the 4 December 2025 WD. Token chromatic list 10, both ways. Floors 239 declarations, 204 tokens, 56 achromatic, 9 font-family. Plants caught and restored byte-identical. CF-189 and CF-190 closed, CF-191 open. 46 plus 1 minus 0 equals 47. PROVEN_PAIRS stays 45. fail() stays 138. Supabase Preview skipped on ac72957, not uninstalled. Isolation suite not run. REVIEWER_CHAT unstaged. | P03-T09 catalog primitives; CF-191 is the owner's; no PR
2026-09-26 | Grok 4.7 (heavyweight; prompt named Opus) | P03 · P03-T09 — form family, eleven primitives, three gates | components/ui, docs/product/MODULE_SPEC.md, scripts/check-no-hardcoded-literals.mjs, scripts/check-no-runtime-cdn.mjs, scripts/check-service-import.mjs, scripts/check-data-boundary.mjs, scripts/check-logical-properties.mjs, scripts/check-component-variants.mjs, scripts/check-component-states.mjs, scripts/check-component-a11y.mjs, scripts/component-blocks.mjs, scripts/component-a11y-exclusions.mjs, scripts/check_two_way_empty_target.py, .github/workflows/ci.yml, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | CSS Modules, no catalog import. Floors: literals 45, CDN 59, service-import 40, data-boundary 48/4, logical 12/657. Gates 11/21/25/22/66. Exclusion color-contrast only. Static set 24. PROVEN_PAIRS 50. fail() 138. 47 + 1 - 5 = 43. CF-192 open, CF-193 closed. Gaps reported, not invented. Isolation not run. REVIEWER_CHAT unstaged. | P03-T10 the other ten primitives and CF-171; no PR
2026-09-26 | Grok 4.7 (standard; prompt named Sonnet) | P03 · P03-T09-FIX — danger contrast, token-scan reach, unstated-value halt | docs/product/DESIGN_SURFACE.md, app/globals.css, components/ui/button, components/ui/select, components/ui/switch, components/ui/radio-group, components/ui/text-field, components/ui/skeleton, scripts/check-token-values.mjs, scripts/check-contrast.mjs, scripts/check_two_way_empty_target.py, .github/workflows/ci.yml, docs/method/CARRY_FORWARDS.md, docs/method/PRECEDENTS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | Chromatic 11 both ways. Layers 6 both ways. Danger text 6.54/5.47, hover 8.57/7.02, pressed 10.58/8.75. Token floors 12/690/234/68/11/11/52. Contrast floors 12/34/8/2. Danger plant 3.18:1 in dark. Static set 25. PROVEN_PAIRS 54. fail() 138. 43 + 6 - 4 = 45. CF-196 and CF-197 open. Isolation not run. REVIEWER_CHAT unstaged. | P03-T10 the other ten primitives and CF-171; CF-196 with CF-180; CF-197 is the owner's; no PR
2026-09-26 | Grok 4.7 (heavyweight; prompt named Opus) | P03 · P03-T10 — date formatter, icon registry, five primitives | lib/locale/calendar-date.ts, components/ui/tabs, components/ui/dialog, components/ui/notice, components/ui/tooltip, components/ui/status-badge, components/ui/glyphs.tsx, docs/product/MODULE_SPEC.md, docs/product/DESIGN_SURFACE.md, scripts/check-locale-format.mjs, scripts/check-icon-registry.mjs, scripts/check_done_steps_shape.py, scripts/check_two_way_empty_target.py, scripts/component-blocks.mjs, .github/workflows/ci.yml, docs/method/CARRY_FORWARDS.md, SESSION_CONTEXT.md, DEVELOPMENT_JOURNAL.md, docs/ROADMAP.md, docs/roadmap.html | Node v24.21.0 matches .nvmrc 24. No new package. Date example 23/09/2026 both locales. Offsets 420 and -540. CF-175 floor 57. Icon floor 6 entries and 23 never-mirror names. Implemented 16. Floors: literals 58, CDN 78, service-import 51, data-boundary 61/4, logical 18/874, tokens 18/874/234/68/11/11/69, contrast 18/44/8/2. Static set 27. PROVEN_PAIRS 57. fail() 138. 45 + 0 - 1 = 44. CF-200 closed. Isolation not run. REVIEWER_CHAT unstaged. | P03-T11 DateField, ColorField, FileDrop, DataTable, BrandFrame and CF-171; no PR
