...
> **Corrected 2026-08-05 — P02-T07.** This section described a client-side port
> with no server, no auth and no database until a Phase 10 that this project's
> real phase plan (`BUILD_PHASES.md`) does not contain. Verified against the
> live tree rather than rewritten from memory: five of the six rows below
> stated something false or materially stale at HEAD — database, migrations,
> types, the privileged client and authentication scope are each already
> delivered by P01 or signed by an OD; the zod row is restated for precision,
> not contradicted. Landed as CF-135.

| BETK rule | Status here | Delivered / in force |
|---|---|---|
| RLS strategy per table; default-deny at the DB | **IN FORCE.** Row-level security is enabled and default-deny on every `public` table, live in `b2s-production` | P01 (P01-T02; `SECURITY_MODEL.md` §2, the tenant-isolation suite) |
| Migration discipline; one applier per environment | **IN FORCE.** 14 migrations, one applier — the Supabase CLI against ADR-012's single environment. ADR-006's "verbatim" migration-to-schema split is asserted on every push | P01 (P01-T02; `scripts/check_migration_split.py`) |
| Types generated from live schema; drift job | **IN FORCE.** `types/database.ts` is generated by the Supabase CLI, never hand-authored; `ci.yml`'s `types-drift` job fails the pipeline on any divergence, including an absent secret | P01 (P01-T02; ADR-002) |
| Privileged-client physical quarantine (`service.ts` + CI guard) | **IN FORCE.** `lib/supabase/server-only/service.ts` is the one construction site; `check-service-import.mjs` fails the build on any import from `app/`, `features/` or `components/`, and on the quarantine directory's own absence | P01 (P01-T02; ADR-005) |
| Zod-on-every-server-action CI guard | **Adapted, not yet reached.** `zod` is installed and `check-zod-coverage` is wired and scoped to `data/` and `brand/`, but no `features/<module>/actions.ts` exists yet to validate. The description naming `BrandConfig`, imported legacy data and preset JSON described the retired client-side plan; the live pattern is ADR-010's per-action validation, owed by the first module that writes one | P02 (`SESSION_CONTEXT.md`: "P02, whose target is the first `features/<module>/actions.ts`") |
| Auth gates, verified-phone, OAuth | **Mixed, not uniformly out of scope.** `SCOPE.md` module 01 (Auth & Access) is Release 1 and under active build. Email-and-password and Google OAuth coexist as signed sign-in mechanisms — OAuth is **in scope**, not deferred. Route- or UI-level auth gates are not yet implemented; RLS at the data layer is the enforced boundary today (`SECURITY_MODEL.md` §2). **Verified-phone remains out of scope** — no signed decision authorises it, and OD-G13's acceptance invariant is satisfied by a verified email address "and nothing else" | P02, in progress (OD-G13, OD-G14, OD-G15, OD-G16) |

**Standing carry-forward, logged from day one:** *"When P10 begins, re-read `DEV_OS_REFERENCE.md` §4 and §6 in full and reinstate the deferred rule layer before the first schema task."* This item lives in every phase's entry checklist so it cannot be lost. The phase plan this file's header names does not contain a Phase 10; this carry-forward is CF-01 and its owner has already been reassigned once — see the ledger.
